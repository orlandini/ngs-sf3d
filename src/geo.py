...
def GenMesh(d_box,l_domain,r_cyl,d_pml, el_core,el_clad,filename):
    ...
    cube_front = Box(Pnt(-d_box,-d_box,l_domain/2),Pnt(d_box,d_box,(l_domain/2+d_pml)))
    cube_front.faces.name = 'dirichlet_3d'
    cube_front.faces.Min(Z).name = 'default'#revert this one
    cyl_front = Cylinder(Pnt(0,0,l_domain/2),Z, r=r_cyl, h=d_pml)
    pml_clad_front = cube_front - cyl_front
    pml_core_front = cube_front * cyl_front
    pml_clad_front.mat("pml_clad_front").maxh = el_clad
    pml_core_front.mat("pml_core_front").maxh = el_core

    cube_back = Box(Pnt(-d_box,-d_box,-(l_domain/2+d_pml)),Pnt(d_box,d_box,-(l_domain/2)))
    cube_back.faces.name = 'dirichlet_3d'
    cube_back.faces.Max(Z).name = 'default'#revert this one
    cyl_back = Cylinder(Pnt(0,0,-l_domain/2-d_pml),Z, r=r_cyl, h=d_pml)
    pml_clad_back = cube_back - cyl_back
    pml_core_back = cube_back * cyl_back
    pml_clad_back.mat("pml_clad_back").maxh = el_clad
    pml_core_back.mat("pml_core_back").maxh = el_core

    domain_list = domain_list + [pml_clad_back, pml_core_back, pml_clad_front, pml_core_front]
